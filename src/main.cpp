 ...
 * NVidia: https://on-demand.gputechconf.com/gtc/2014/presentations/S4385-order-independent-transparency-opengl.pdf
...
int main() {
    ...
    // Create shaders
    OPAQUE_SHADER(shaders) = new DefaultShader(
        "res/shaders/solid/shader.vert",
        "res/shaders/solid/shader.frag"
    );

    COUNT_SHADER(shaders) = new DefaultShader(
        "res/shaders/solid/shader.vert",
        "res/shaders/transparent/count.frag"
    );

    TRANSPARENT_SHADER(shaders) = new DefaultShader(
        "res/shaders/solid/shader.vert",
        "res/shaders/transparent/shader.frag"
    );

    COMBINE_SHADER(shaders) = new DefaultShader(
        "res/shaders/combine/shader.vert",
        "res/shaders/combine/shader.frag"
    );
    ...
    for (auto& shader : shaders) {
        ...
        if ((shader == TRANSPARENT_SHADER(shaders)) || (shader == COMBINE_SHADER(shaders))) {
            shader->setUniformUInt("screenWidth", RES_X);
        }

        if ((shader == OPAQUE_SHADER(shaders)) || (shader == TRANSPARENT_SHADER(shaders))) {
        ...
        if ((shader == COUNT_SHADER(shaders)) || (shader == TRANSPARENT_SHADER(shaders))) {
    ...
    QUAD_POS(dBuffers) = new BufferData(sizeof(quadPosition), (void*)quadPosition);
    ...
    while (!window.shouldClose()) {
        ...
        // Rotate dragon
        DRAGON(models)->transform.rotateAxis(vec3(0.0f, 1.0f, 0.0f), -deltaTime*3.0f);
        ...
        if (GUI::isButtonDown(1)) {
            ...
            glm::vec3 cameraRotation = glm::vec3(
                radians(-deltaPosY * mouseSpeed),
                radians(-deltaPosX * mouseSpeed * 0.666f),
                0.0f
            );
        ...
        ImGui::DragFloat("Mouse Speed", &mouseSpeed, 0.4f, 0.1f, 10.0f, "%.3f", 1.0f);
        ImGui::DragFloat("Movement Speed", &movementSpeed, 0.2f, 100.0f, 300.0f, "%.3f", 1.0f);
