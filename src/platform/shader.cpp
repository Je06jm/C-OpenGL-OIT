...
Shader::Shader() { this->program = glCreateProgram(); }
...
void Shader::setUniformUIntv(const string &name, unsigned int *pointer,
                             size_t count) {
  GLint uniform = this->getUniformLocation(name);
...
void Shader::setUniformFloatv(const string &name, float *pointer,
                              size_t count) {
  GLint uniform = this->getUniformLocation(name);
