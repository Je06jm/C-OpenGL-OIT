...
BufferArray::BufferArray() { glGenVertexArrays(1, &this->vao); }

BufferArray::~BufferArray() { glDeleteVertexArrays(1, &this->vao); }
...
void BufferArray::setAttribute(unsigned int attribute, unsigned int components,
                               GLenum type, size_t stride, size_t offset) {
  // Records the attribute and tell OpenGL about it
  this->attributes |= (1 << attribute);

  glVertexAttribPointer(attribute, components, type, GL_FALSE, stride,
                        (void *)offset);
  glEnableVertexAttribArray(attribute);
}

void BufferArray::setAttributeI(unsigned int attribute, unsigned int components,
                                GLenum type, size_t stride, size_t offset) {
...
Buffer::Buffer() { glGenBuffers(1, &this->buffer); }

Buffer::~Buffer() { glDeleteBuffers(1, &this->buffer); }
...
void BufferData::bind() { glBindBuffer(GL_ARRAY_BUFFER, this->buffer); }
...
void BufferStorage::unbind() { glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0); }
...
void BufferStorage::clear() {
  glClearNamedBufferData(this->buffer, GL_R8UI, GL_RED_INTEGER,
...
void BufferCounter::unbind() { glBindBuffer(GL_ATOMIC_COUNTER_BUFFER, 0); }
