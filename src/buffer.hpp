...
public:
     ...
     * @param components The number of components each item has. (Vec3 has 3 components)
     * @param type The data type (FLOAT, DOUBLE, etc.)
     * @param stride The stride between items. A stride of zero will use the default stride used by OpenGL
     * @param offset The offset from the start of the attribute data
    */
    void setAttribute(unsigned int attribute, unsigned int components, GLenum type, size_t stride, size_t offset);
     ...
     * @param offset The offset from the start of the attribute data
    */
    void setAttributeI(unsigned int attribute, unsigned int components, GLenum type, size_t stride, size_t offset);
...
public:
     ...
     * @param usage OpenGL usage hint (GL_MAP_WRITE_BIT | GL_MAP_READ_BIT | GL_MAP_PERSISTENT_BIT | GL_MAP_COHERENT_BIT)
    */
    BufferCounter(
        unsigned int resetValue = 0,
        GLbitfield usage = GL_MAP_WRITE_BIT | GL_MAP_READ_BIT | GL_MAP_PERSISTENT_BIT | GL_MAP_COHERENT_BIT
    );
